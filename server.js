...
app.get('/course', (req, res) => {
  ...
  return fetchAndRenderCourse(courseId, res, req.headers['user-agent']);
});

app.get('/course/:courseId', (req, res) => {
  return fetchAndRenderCourse(req.params.courseId, res, req.headers['user-agent']);
});

async function fetchAndRenderCourse(courseId, res, ua) {
  ...
  try {
    ...
    if (!doc.exists) {
      return res.status(404).send(courseNotFoundHtml(store));
    }
    ...
    res.send(courseHtml({ courseId, courseName, ogDescription, placeNames, placeCount, placesCoords, store }));
  } catch (err) {
    console.error(err);
...
// ─── 스토어 링크 (요청 UA 기준) ───────────────────────────────────────────────
// 안드로이드 출시(2026-07-30) 이후 — 딥링크가 실패했을 때 기기에 맞는 스토어로 보낸다.
// 이 페이지는 서버 렌더링이라 UA로 미리 고를 수 있고, 그래야 openApp()의 자동
// 폴백(storeBtn.click())도 안드로이드에서 App Store로 끌고 가지 않는다.
const STORE_IOS     = 'https://apps.apple.com/kr/app/%EB%96%A0%EB%82%98/id6767218543';
const STORE_ANDROID = 'https://play.google.com/store/apps/details?id=com.seoktaedev.tteona';
const SVG_APPLE = '<svg viewBox="0 0 814 1000" xmlns="http://www.w3.org/2000/svg"><path d="M788.1 340.9c-5.8 4.5-108.2 62.2-108.2 190.5 0 148.4 130.3 200.9 134.2 202.2-.6 3.2-20.7 71.9-68.7 141.9-42.8 61.6-87.5 123.1-155.5 123.1s-85.5-39.5-164-39.5c-76 0-103.7 40.8-165.9 40.8s-105-37.5-155.5-127.4C46 790.7 0 663.5 0 541.8c0-207.8 134.9-317.9 267.9-317.9 75.3 0 137.9 49.2 184.2 49.2 44.4 0 114.5-52.1 201.5-52.1zM568.5 81.4C588.5 55.4 602.8 19.9 602.8 0c0-1.9-.1-3.8-.3-5.7-30.5 1.1-67.1 20.4-89.3 47.9-17.9 21.3-35.5 57.2-35.5 87.2 0 2 .3 4 .5 4.7 2 .3 4.1.5 6.1.5 28.3 0 63.2-18.8 84.2-53.2z"/></svg>';
const SVG_PLAY  = '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M17.6 9.48l1.84-3.18c.16-.31.04-.69-.26-.85-.29-.15-.65-.06-.83.22l-1.88 3.24c-2.86-1.21-6.08-1.21-8.94 0L5.65 5.67c-.19-.29-.58-.38-.87-.2-.28.18-.37.54-.22.83L6.4 9.48C3.3 11.25 1.28 14.44 1 18h22c-.28-3.56-2.3-6.75-5.4-8.52zM7 15.25c-.69 0-1.25-.56-1.25-1.25s.56-1.25 1.25-1.25 1.25.56 1.25 1.25-.56 1.25-1.25 1.25zm10 0c-.69 0-1.25-.56-1.25-1.25s.56-1.25 1.25-1.25 1.25.56 1.25 1.25-.56 1.25-1.25 1.25z"/></svg>';

function storeFor(ua) {
  return /android/i.test(String(ua || ''))
    ? { url: STORE_ANDROID, label: 'Google Play에서 다운로드', svg: SVG_PLAY }
    : { url: STORE_IOS,     label: 'App Store에서 다운로드',   svg: SVG_APPLE };
}

function courseHtml({ courseId, courseName, ogDescription, placeNames, placeCount, placesCoords = [], store = storeFor('') }) {
...
<body>
  ...
  <div class="cta">
    ...
    <a class="store-btn" id="storeBtn" href="${store.url}">
      ${store.svg}
      ${store.label}
    </a>
  </div>
...
function courseNotFoundHtml(store = storeFor('')) {
  return `<!DOCTYPE html>
<html lang="ko">
<head>
  <meta charset="UTF-8">
...
<body>
  <div class="bottom">
    ...
    <a class="store-btn" href="${store.url}">
      ${store.svg}
      ${store.label}
    </a>
  </div>
